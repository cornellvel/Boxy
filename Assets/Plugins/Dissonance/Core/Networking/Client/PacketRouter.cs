...
namespace Dissonance.Networking.Client
{
    ...
    internal class PacketRouter
    {
        ...
        internal TrafficCounter RecvRoutingUpdate { get; private set; }
        internal TrafficCounter RecvVoiceData { get; private set; }
        internal TrafficCounter RecvTextData { get; private set; }
        internal TrafficCounter RecvHandshakeResponse { get; private set; }
        #endregion
        ...
        private bool CheckSessionId(ref PacketReader reader)
        {
            ...
            if (_connection.SessionId != session)
            {
                Log.Warn("Received a packet with incorrect session ID. Expected {0}, got {1}", _connection.SessionId, session);
                return false;
            }
        ...
        private void HandleErrorWrongSession(ref PacketReader reader)
        {
            var session = reader.ReadErrorWrongSession();
            if (session != _connection.SessionId)
                _error = "Incorrect session";
        }
