...
import UnityEngine.Networking;
	...
	function Start() {
		...
		var avatar = this.transform.parent.gameObject.GetComponent.<NetworkIdentity>();

		if (!(avatar.isLocalPlayer)) {
			Debug.Log(avatar.isLocalPlayer);
			return;
		}

		target = GameObject.Find("left_offset").transform;
		//elbowTarget = GameObject.Find("ElbowTarget_R").transform;

		//elbowTarget = GameObject.Find("ElbowTarget_L").transform;
		Debug.Log("left controller position");
		Debug.Log(target.position);
		Debug.Log("-------------------");
		Debug.Log("left hand position");
